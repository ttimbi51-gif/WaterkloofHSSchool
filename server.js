...
app.get('/api/billboard-images', async (req, res) => {
  try {
    const billboardDir = path.join(__dirname, 'billbord');
    await fssync.promises.mkdir(billboardDir, { recursive: true });
    const files = await fs.readdir(billboardDir);
...
app.get('/api/shared-slides', async (req, res) => {
  try {
    ...
    const imageDirectories = [path.join(__dirname, 'billbord'), SLIDES_DIR];
    const imageUrls = [];
    const seen = new Set();
    ...
    const slides = (imageUrls.length ? imageUrls : [fallbackImage]).map((image, index) => ({
      image,
      title: fallbackTitle,
...
app.get('/api/admin/billboard-media', async (req, res) => {
  ...
  try {
    ...
    for (const file of imageFiles) {
      const stats = await fs.stat(path.join(billboardDir, file));
      media.push({
        fileName: file,
...
app.post('/api/admin/upload-image', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    await appendActivityLog({
      ...
      displayName: account.displayName,
      role: account.role,
      action: 'uploaded-billboard-image',
      fileName: safeName,
      ip: req.ip,
    });
    res.json({ success: true, imageUrl: `/billbord/${encodeURIComponent(safeName)}` });
  } catch (error) {
...
app.delete('/api/admin/billboard-image', async (req, res) => {
  ...
  try {
    ...
    const targetPath = path.join(__dirname, 'billbord', safeName);
