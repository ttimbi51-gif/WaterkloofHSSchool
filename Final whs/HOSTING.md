...
### Admin accounts
Set these Azure Application Settings with private values before deployment:

```env
WHS_PASSWORD=your-private-password
ANNELE_PASSWORD=your-private-password
NONG_PASSWORD=your-private-password
IT_PASSWORD=your-private-password
```

The built-in local defaults are intended only for local testing. Change them in Azure immediately.

### Backups and cloud storage
For production, configure Azure Blob Storage or another managed database for `applications`, `data`, `slides` and `billbord` uploads. The current file-based storage is suitable for a small deployment but must be backed up regularly and may not survive every hosting migration.

### PowerPoint files
PowerPoint files need a public HTTPS `SITE_ROOT` for the Office web viewer. For a fully self-contained billboard, convert PPT/PPTX uploads to PDF or slide images during deployment using LibreOffice or an Azure conversion service.
