...
async function loadBillboardSlides() {
  try {
    ...
    if (Array.isArray(sharedSlides) && sharedSlides.length > 0) {
      ...
      const audioSlide = sharedSlides.find((slide) => slide.kind === 'audio');
      if (audioSlide) backgroundAudioUrl = audioSlide.media;
    }
  } catch (error) {
...
const presentationDownload = document.getElementById('presentation-download');
...
function showSlide(index) {
  ...
  if (slidePresentation) {
    ...
    if (presentationDownload) {
      presentationDownload.href = slide.kind === 'presentation' ? slide.media : '#';
    }
  }
...
window.addEventListener('load', async () => {
  ...
  if (audio && audioToggle) {
    ...
    audioToggle.addEventListener('click', startBackgroundAudio);
