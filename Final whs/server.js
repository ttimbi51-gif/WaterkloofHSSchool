...
const upload = multer({
  ...
  limits: { fileSize: 95 * 1024 * 1024, files: 20 },
});
...
const ADMIN_ACCOUNTS = {
  WHS: {
    password: process.env.WHS_PASSWORD || 'Waterkloof2026',
    displayName: 'Waterkloof Hills School Admin',
    role: 'school_admin',
    allowedSections: null,
  },
  ANNELE: {
    password: process.env.ANNELE_PASSWORD || 'Annele2026',
    displayName: 'Annele Web Admin',
    role: 'web_admin',
    allowedSections: ['home', 'staff', 'billboard', 'media', 'slides'],
  },
  NONG: {
    password: process.env.NONG_PASSWORD || 'NONG2026',
    displayName: 'NONG Web Admin',
    role: 'web_admin',
    allowedSections: ['home', 'staff', 'billboard', 'media', 'slides'],
  },
  TSHIRELETSO: {
...
const getAdminAccount = (username, password, req) => {
  const sessionAccount = req ? getSessionAccount(req) : null;
...
const getSessionAccount = (req) => {
  const token = req.cookies?.whss_admin_session;
...
app.get('/api/admin/activity-log', async (req, res) => {
  ...
  const account = getAdminAccount(username, password, req);
  if (!account) {
...
app.post('/api/admin/login', async (req, res) => {
  ...
  adminSessions.set(token, { account, expiresAt: Date.now() + SESSION_TTL_MS });
  const secureCookie = req.secure || req.headers['x-forwarded-proto'] === 'https' ? '; Secure' : '';
  res.setHeader('Set-Cookie', `whss_admin_session=${token}; Max-Age=${SESSION_TTL_MS / 1000}; HttpOnly; SameSite=Lax${secureCookie}`);
...
const buildApplicationPdf = (fields) => {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.2);
    doc.fontSize(11).text('Tel: 076 809 0560 / 014 008 5317 · Email: admin@waterkloofhillsschool.co.za', { align: logoPlaced ? 'left' : 'center' });
...
app.post('/api/create-application', upload.any(), async (req, res) => {
  try {
    ...
    for (const file of req.files || []) {
      const safeFileName = path.basename(String(file.originalname || 'upload'));
      const dest = path.join(appPath, safeFileName);
